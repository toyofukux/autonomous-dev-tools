...
### `external-research`
A second researcher that complements `codebase-researcher` — fetches relevant docs, library references, recent best practices, blog posts. Triggered from `/ad-spec` when the change involves an unfamiliar API or pattern. Uses Context7 MCP and WebFetch.
...
### `migrations-curator`
A subagent specialized in safe database migration design — checks lock impact, proposes online-DDL alternatives, designs two-phase rollouts. Plugs into `/ad-spec` and `/ad-validate`.
...
### `/ad-loop --parallel` actually parallel
...
### `/ad-budget`
Per-feature cost tracking. Each iteration logs token usage and wall time; `/ad-budget` summarizes.
...
### Background monitors
A `monitors/` config that watches the dev server's error log and pings the main thread when an unexpected error surfaces during `/ad-dev`.
