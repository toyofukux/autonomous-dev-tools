...
When [[ad-validate]] or [[ad-verify]] reports failure, route the findings to the right agent and re-run only the necessary part of the chain.
...
The spec's `attempts` frontmatter counter tracks this. [[ad-validate]] increments it on FAIL.
...
3. **For Attempt 1**:
   ...
   - When the builder finishes, mark `status: verifying` and tell the user to re-run [[ad-verify]] → [[ad-validate]].
4. **For Attempt 2**:
...
5. **For Attempt 3**: stop. Output a clear prompt:
   ...
   > - **fix-code** — last validator report attached; reopen [[ad-fix]] only after manual changes
   > - **fix-spec** — edit `specs/iterations/$spec_id.md` directly, then re-run [[ad-dev]]
   > - **drop** — abandon this iteration (`/ad-pr --abandon $spec_id`)
...
- **Do not chain into [[ad-verify]] or [[ad-validate]] automatically.** The user decides when to re-validate.
