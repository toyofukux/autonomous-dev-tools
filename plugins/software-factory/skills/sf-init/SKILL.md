...
Run once, at the very start of a project, before any feature work. If `specs/` already exists, refuse and suggest [[sf-bootstrap]] instead.

## What you do

1. **Check preconditions**:
   - If `specs/` exists, stop and tell the user to use `/sf-bootstrap`.
...
5. **Do not create**: `specs/units/`, `specs/guidelines/`. These appear only when [[sf-organize]] or [[sf-guideline]] is run.
...
7. **Summarize**: print the file tree you created and the next recommended command (`/sf-loop "describe your first feature"` or `/sf-story` if they want stepwise).

## Discipline

- **Progressive disclosure**: do not create folders the user did not ask for. If the user mentions wanting guidelines, mention `/sf-guideline <name>` instead of pre-creating them.
