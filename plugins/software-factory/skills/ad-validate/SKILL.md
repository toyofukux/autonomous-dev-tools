...
- `specs/iterations/$spec_id.md` exists with `status: validating` (i.e., [[ad-verify]] passed or marked uncoverable AC).
...
5. **Update spec frontmatter** based on verdict:
   - PASS → `status: pr` (ready for [[ad-pr]])
...
6. **Present the report** to the user. On FAIL:
   ...
   - If `attempts >= 3`, **always** stop and require human decision (see [[ad-fix]] for the 3-attempt rule)
7. **On PASS**, next command: `/ad-pr $spec_id`.

## Discipline

- **The validator is honest.** If it returns "no issues", trust it and proceed. Do not pad findings.
- **Do not invoke any builder from here.** That's [[ad-fix]]'s job.
- **Capture the validator's CLAUDE.md suggestions verbatim** so [[ad-pr]] can include them in the PR body.
